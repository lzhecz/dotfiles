...
local function autostart(dirs)
    ...
    spawn.easy_async_with_shell('command -v '..cmd.exe, function(path)
        ...
        spawn.with_shell("~/.config/autostart/autostart.sh")
        spawn.with_shell([[setxkbmap -layout "us,ru" -option "grp:win_space_toggle"]])
        spawn.with_shell([[setxkbmap -layout "us,ru" -option "grp:alt_shift_toggle"]])
    end)
end
