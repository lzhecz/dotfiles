...
spawn.easy_async_with_shell('command -v rofi', function(path)
    ...
    local keys = {
        awful.key({ mod.alt            }, 'space', function() awful.spawn(cmd.drun) end,
                  {description = 'application search', group = 'external'}),
        awful.key({ mod.super            }, '\\', function() awful.spawn(cmd.drun) end,
                  {description = 'application search', group = 'external'}),
        awful.key({ mod.super  }, 'r', function() awful.spawn(cmd.run) end,
